...
namespace embree
{
  ...
  void TutorialApplication::renderToFile(const FileName& fileName)
  {
    ...
    // SHADER_DEPTH post-processing
    if (shader == SHADER_DEPTH) {
      std::cout << "Post processing depth\n";
      float min_value =   std::numeric_limits<float>::infinity();
      float max_value =   -std::numeric_limits<float>::infinity();
      for (int i = 0; i < width * height; ++i) { 
        float value = *reinterpret_cast<float*>(&pixels[i]);
      ...
      for (int i = 0; i < width * height; ++i) {
        float pixel_value = *reinterpret_cast<float*>(&pixels[i]);
        float final_value =  1.0;
        if (pixel_value != std::numeric_limits<float>::infinity()) 
          final_value = (pixel_value - min_value) / (max_value- min_value);
        final_value = 1.0 - final_value;
        unsigned int value = (unsigned int) (255.0f * clamp(final_value,0.0f,1.0f));
