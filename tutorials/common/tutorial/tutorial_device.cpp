...
/* renders a single pixel with geometry normal shading */
float renderPixelDepth(float x, float y, const ISPCCamera& camera, RayStats& stats)
{
  /* initialize ray */
  Ray ray;
  ray.org = Vec3fa(camera.xfm.p+x*camera.xfm.l.vx + y*camera.xfm.l.vy);
  //ray.dir = Vec3fa(normalize(x*camera.xfm.l.vx + y*camera.xfm.l.vy + camera.xfm.l.vz));
  ray.dir = Vec3fa(normalize(camera.xfm.l.vz));
  ray.tnear() = 0.0f;
  ray.tfar = inf;
  ray.geomID = RTC_INVALID_GEOMETRY_ID;
  ray.primID = RTC_INVALID_GEOMETRY_ID;
  ...
  /* shade pixel */
  if (ray.geomID == RTC_INVALID_GEOMETRY_ID) return std::numeric_limits<float>::infinity();
  else return ray.tfar;
}

void renderTileDepth(int taskIndex,
                  int threadIndex,
                  int* pixels,
                  const unsigned int width,
                  const unsigned int height,
                  ...
                  const int numTilesY)
{
  ...
  for (unsigned int y=y0; y<y1; y++) for (unsigned int x=x0; x<x1; x++)
  {
    float depth  = renderPixelDepth((float)x,(float)y,camera,g_stats[threadIndex]);
    pixels[y*width+x] = *reinterpret_cast<int*>(&depth);
  }
}

/* renders a single pixel with geometry normal shading */
Vec3fa renderPixelCurvature(float x, float y, const ISPCCamera& camera, RayStats& stats)
{
  /* initialize ray */
  Ray ray;
  ray.org = Vec3fa(camera.xfm.p+x*camera.xfm.l.vx + y*camera.xfm.l.vy);
...
void renderTileCurvature(int taskIndex,
                  int threadIndex,
                  int* pixels,
                  const unsigned int width,
                  const unsigned int height,
                  ...
                  const int numTilesY)
{
  ...
  for (unsigned int y=y0; y<y1; y++) for (unsigned int x=x0; x<x1; x++)
  {
    Vec3fa color= renderPixelCurvature((float)x,(float)y,camera,g_stats[threadIndex]);
    unsigned int r = (unsigned int) (255.0f * clamp(color.x,0.0f,1.0f));
    unsigned int g = (unsigned int) (255.0f * clamp(color.y,0.0f,1.0f));
    unsigned int b = (unsigned int) (255.0f * clamp(color.z,0.0f,1.0f));
    pixels[y*width+x] = (b << 16) + (g << 8) + r;
                  ...
                  const int numTilesY)
{
  ...
  for (unsigned int y=y0; y<y1; y++) for (unsigned int x=x0; x<x1; x++)
  {
    Vec3fa color= renderPixelDiffuseAlbedo((float)x,(float)y,camera,g_stats[threadIndex]);
    unsigned int r = (unsigned int) (255.0f * clamp(color.x,0.0f,1.0f));
    unsigned int g = (unsigned int) (255.0f * clamp(color.y,0.0f,1.0f));
    unsigned int b = (unsigned int) (255.0f * clamp(color.z,0.0f,1.0f));
    pixels[y*width+x] = (b << 16) + (g << 8) + r;
