...
namespace embree
{
  ...
  bool IsObtuse(const SceneGraph::TriangleMeshNode::Vertex& v0,
                const SceneGraph::TriangleMeshNode::Vertex& v1,
                const SceneGraph::TriangleMeshNode::Vertex v2) {
      float a = distance(v1, v0);
      float b = distance(v2, v0);
      float c = distance(v2, v1);
      float a2 = a * a, b2 = b * b, c2 = c * c;
      return (a2 +b2 < c2) || (b2 +c2 < a2) || (c2 +a2 < b2);
  }


  void SceneGraph::TriangleMeshNode::calculateCurvatures(int s) {
    if (positions.size() == 0) return;
    if (numVertices() == 0) return;
    //curvatures.resize(numVertices(), 2.0f * std::acos(-1.0f));
    Eigen::MatrixXf V(numVertices(), 3);
    Eigen::MatrixXi F(numPrimitives(), 3);
    Eigen::VectorXf K;
    for (int i = 0; i < numPrimitives(); ++i) {
      F(i, 0) = triangles[i].v0;
      F(i, 1) = triangles[i].v1;
      F(i, 2) = triangles[i].v2;
    }
    for (int i = 0; i < numVertices(); ++i) {
      V(i, 0) = positions[s][i].x;
      V(i, 1) = positions[s][i].y;
      V(i, 2) = positions[s][i].z;
    }
    igl::gaussian_curvature(V,F,K);
    Eigen::MatrixXf C;
    igl::jet(K, true, C);
    curvatures.resize(numVertices() * 3);
    for (int i = 0; i < numVertices(); ++i) {
      curvatures[3 * i + 0] = C(i, 0);
      curvatures[3 * i + 1] = C(i, 1);
      curvatures[3 * i + 2] = C(i, 2);
    }
    return;
    std::vector<float> local_areas(numVertices(), 0.0f);
    for (const auto& t : triangles) {
      SceneGraph::TriangleMeshNode::Vertex v0 = positions[s][t.v0];
      SceneGraph::TriangleMeshNode::Vertex v1 = positions[s][t.v1];
      SceneGraph::TriangleMeshNode::Vertex v2 = positions[s][t.v2];
      SceneGraph::TriangleMeshNode::Vertex u = normalize(v1 - v0);
      SceneGraph::TriangleMeshNode::Vertex v = normalize(v2 - v0);
      SceneGraph::TriangleMeshNode::Vertex w = normalize(v2 - v1);
      float dot_uv = dot(u, v);
      float dot_uw = dot(w, u);
      float dot_vw = dot(v, w);
      float area = 0.5f * length(cross(u, v));
      curvatures[t.v0] -= std::acos(dot_uv);
      curvatures[t.v1] -= std::acos(dot_uw);
      curvatures[t.v2] -= std::acos(dot_vw);
      if (IsObtuse(v0, v1, v2)) {
        local_areas[t.v0] += (dot_uv < 0.0f ? area / 2.0f : area / 4.0f);
        local_areas[t.v1] += (dot_uw < 0.0f ? area / 2.0f : area / 4.0f);
        local_areas[t.v2] += (dot_vw < 0.0f ? area / 2.0f : area / 4.0f);
      } else {
        float cot_uv = dot_uv / sqrt(1.0f - dot_uv * dot_uv);
        float cot_uw = dot_uw / sqrt(1.0f - dot_uw * dot_uw);
        float cot_vw = dot_vw / sqrt(1.0f - dot_vw * dot_vw);
        local_areas[t.v0] += 0.125f * (cot_uw + cot_vw) * length(w);
        local_areas[t.v1] += 0.125f * (cot_vw + cot_uv) * length(v);
        local_areas[t.v2] += 0.125f * (cot_uw + cot_uv) * length(u);
      }
    }
    //for (int i = 0; i < curvatures.size(); ++i)
      //curvatures[i] /= local_areas[i];
  }
