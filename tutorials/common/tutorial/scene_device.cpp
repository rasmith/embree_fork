...
namespace embree
{
    ...
    : geom(TRIANGLE_MESH), positions(nullptr), normals(nullptr)
  {
    ...
    curvatures = (float*) in->curvatures.data();
  ...
  ISPCGeometry* ISPCScene::convertGeometry (TutorialScene* scene, Ref<SceneGraph::Node> in)
  {
    ...
    if (in->geometry) {
      //std::cout << "ISPCSCene: geometry is good, returing\n";
      return (ISPCGeometry*) in->geometry;
    }
    else if (Ref<SceneGraph::TriangleMeshNode> mesh \
          = in.dynamicCast<SceneGraph::TriangleMeshNode>()) {
      //std::cout << "ISPCScene: TriangleMeshNode\n";
